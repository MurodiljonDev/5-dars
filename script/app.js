...
//Uy ishi
todolistTaskContainer.addEventListener("click", editElement)

//  Uy ishi Edit qilish functioni
function editElement(event){
    if(event.target.closest(".edit-btn")){
        const btnWrapper = event.target.closest(".todolist__item")
        btnWrapper.firstElementChild.toggleAttribute("contenteditable", "true")
    }
  ...
  if(ALL_TASKS.length > 0){
    tasks.forEach((taskItem, index) => {
      ...
      taskItemElement.innerHTML = `
        <strong class="${taskItem.isCompleted ? "completed-item" : ""}">${taskItem.taskName}</strong>
